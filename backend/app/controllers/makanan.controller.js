const Tutorial = require("../models/makanan.model.js");
...
// Find a single Tutorial with a id
exports.findOneNama = (req, res) => {
  Tutorial.findByNama(req.params.nama, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `Makanan Dengan Nama ${req.params.nama} Tidak Ditemukan.`
          });
        } else {
          res.status(500).send({
            message: "Error retrieving Tutorial with id " + req.params.nama
          });
        }
      } else res.send(data);
  });
